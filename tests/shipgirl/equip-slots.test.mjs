...
test('formatMountProgression: constant, increasing, aircraft wording', () => {
  ...
  assert.equal(formatMountProgression(constant, 0, false), '포좌 1');

  const increasing = { '101031': [1, 1, 1], '101032': [1, 1, 1], '101033': [1, 2, 1], '101034': [1, 2, 1] };
  assert.equal(formatMountProgression(increasing, 1, false), '포좌 1 → 2 (한계돌파 2)');

  assert.equal(formatMountProgression(increasing, 1, true), '함재기 1 → 2 (한계돌파 2)');
});
...
test('renderEquipSlotSection: empty when no weapon slots', () => {
  const ship = { equip_1: [10], equipment_proficiency: [0.3], base_list: { '1': [1, 1, 1] } };
