...
class AzurLaneDataProcessor:
    ...
    def replace_name_codes(self, text: Any, name_code_dict: Dict[str, str]) -> Any:
        ...
        if isinstance(text, dict):
            return {
                key: self.replace_name_codes(value, name_code_dict)
    ...
    def translate_text_safe(self, text: str, source_key: str) -> str:
        ...
        try:
            translated = ts.translate_text(
                text, translator="google", from_language="zh-CN", to_language="ko"
            )
    ...
    def process_activity_data(self) -> None:
        ...
        try:
            ...
            self.activity_ins_language_kr_data = self.fetch_json_data(
                self.URLS["activity_ins_language_kr"]
            )
            self.activity_ins_language_cn_data = self.fetch_json_data(
                self.URLS["activity_ins_language_cn"]
            )
            self.activity_ins_npc_template_data = self.fetch_json_data(
                self.URLS["activity_ins_npc_template"]
            )
            ...
            for activity_id, activity_info in activity_ins_data.items():
                ...
                if processed_activity:
                    self.processed_activity_data[processed_activity["id"]] = (
                        processed_activity
                    )
    ...
    def _process_single_activity(
        ...
        # Extract the message ID from message_persist (e.g., "ins_100" -> 100)
        # This is used for commander reply/response keys, as some entries have
        # message_persist IDs that differ from their entry ID
        message_id = activity_id  # Default to activity_id
        if message_persist and isinstance(message_persist, str):
            match = re.match(r"ins_(\d+)", message_persist)
            if match:
        ...
        # Add operation options and replies using message_id for correct key lookup
        self._add_operation_data(processed, message_id)

        return processed

    def _process_reply_groups(
        self, npc_discuss_persist: List[str]
            ...
            for reply_id in npc_reply_persist:
                ...
                if reply_npc_data:
                    ...
                    if reply_message_persist and reply_npc_ship_group:
                        reply_message_text = self.get_localized_text(
                            reply_message_persist
                        )
                        if reply_message_text:
                            reply_process[check_id] = {
                                reply_npc_ship_group: reply_message_text
                            }
                            check_id += 1
    ...
    def _add_operation_data(
        self, processed_activity: Dict[str, Any], message_id: int
    ) -> None:
        """
        ...
        Args:
            ...
            message_id: Message ID extracted from message_persist (e.g., "ins_100" -> 100)
                       Used for constructing commander reply/response keys
        """
        operations = [
            (f"ins_op_{message_id}_1_1", "op_option1", None),
            (f"op_reply_{message_id}_1_1", "op_reply1", "reply1_shipgirl"),
            (f"ins_op_{message_id}_1_2", "op_option2", None),
            (f"op_reply_{message_id}_1_2", "op_reply2", "reply2_shipgirl"),
        ]
        ...
        for lang_key, new_key, shipgirl_key in operations:
            ...
            if shipgirl_key:
                ...
                for npc_id, npc_info in self.activity_ins_npc_template_data.items():
                    if (
                        isinstance(npc_info, dict)
                        and npc_info.get("message_persist") == lang_key
                    ):
    ...
    def finalize_processing(self) -> None:
        ...
        self.name_code_dict = {
            key: value.get("name", "") for key, value in name_code_data.items()
        }

        # Apply name code replacement to all processed data
        self.processed_activity_data = self.replace_name_codes(
            self.processed_activity_data, self.name_code_dict
        )
        ...
        print(
            f"Saved {len(self.processed_activity_data)} processed activities to './output/juustagram_data.json'"
        )
