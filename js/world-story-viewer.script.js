document.addEventListener('DOMContentLoaded', () => {
    ...
    audio.loop = true;
    audio.volume = 0.01; // Default volume set to 10%
    ...
    const switchView = (viewToShow) => {
        ...
        // Stop music when leaving the story viewer
    ...
    function handleBgm(bgmName) {
        if (bgmName && bgmName !== currentBgm) {
            currentBgm = bgmName;
            audio.src = `${BGM_URL_PREFIX}${currentBgm}.ogg`;
            audio.play().catch(e => console.warn("Audio playback failed, user interaction may be required.", e));
    ...
    function populateEventGrid(searchTerm = '') {
        ...
        const filteredEvents = Object.keys(storylineData)
            .filter(key => storylineData[key].name.toLowerCase().includes(searchTerm.toLowerCase()));

        filteredEvents.forEach(key => {
            const event = storylineData[key];
            const card = createCard(
                event.name,
                `Chapter: ${event.name.replace('Chapter', '')}`,
                null, 
                null,
                () => selectEvent(key) 
            );
    ...
    function createCard(title, subtitle, icon, pathPrefix, onClick) {
        ...
        if (icon) {
            ...
            thumbnailHtml = `<div class="card-thumbnail" style="background-color: #34495e; display: flex; justify-content: center; align-items: center; color: white; font-size: 2rem;"><span></span></div>`;
        }
    ...
    function startStory(memory, updateUrl = true) {
        ...
        scriptIndex = 0;
        lastActorId = null;
        currentBgm = null; // Reset BGM for new story
    ...
    function getActorInfo(line) {
        ...
        if (actorInfo.name === '통신기' || actorInfo.name === '분석기' || actorInfo.name === '모두들' || actorInfo.name?.includes('?')) {
            actorInfo.icon = null;
        }
    ...
    function handleOptionSelect(chosenFlag) {
        ...
        for (let i = scriptIndex + 1; i < currentStoryScript.length; i++) {
            if (currentStoryScript[i].optionFlag === chosenFlag) {
                nextIndex = i;
                break;
            }
        }
    ...
    function renderScriptLine() {
        ...
        if (infoText && infoText.trim() !== "") {
            ...
            if (line.subActors) {
                const subActorNames = line.subActors.map(sub => {
                    const subActorInfo = getActorInfo({ actor: sub.actor });
                    return subActorInfo.name;
                });
            ...
            if (actorInfo.id !== lastActorId) {
                ...
                if (line.actorShadow) {
                    actorPortrait.classList.add('actor-shadow');
                } else {
                    actorPortrait.classList.remove('actor-shadow');
                }
        ...
        if (isAtEnd) {
            ...
            nextStoryBtn.classList.toggle('hidden', !nextMemory);
        } else {
    ...
    backToEventBtn.addEventListener('click', (e) => { e.preventDefault(); switchView(eventSelectionView); window.history.pushState({}, '', 'world-story-viewer.html'); });
