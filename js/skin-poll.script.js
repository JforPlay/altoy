document.addEventListener("DOMContentLoaded", () => {
    ...
    const resetFiltersBtn = document.getElementById('reset-filters-btn'); // ADD THIS LINE
    ...
    fetch("data/subset_skin_data.json")
        ...
        .then((jsonData) => {
            ...
            // --- FIX: Removed the forced "L2D" filter to start with "All" ---
            applyFilters();
    ...
    // --- LEADERBOARD LOGIC (No changes here) ---
    ...
    const fetchPollDataForSkins = async (skinIds) => {
        ...
        try {
            ...
            snapshots.forEach(snapshot => { snapshot.forEach(doc => { pollDataMap[doc.id] = doc.data(); }); });
    ...
    const renderPollList = (skinsToRender) => {
        ...
        skinsToRender.forEach((skin) => {
            ...
            const resultsText = skin.total_votes > 0 ? `평균 점수: ${skin.average_score.toFixed(2)} / 5 (${skin.total_votes} 표)` : "아직 투표가 없습니다.";
            pollBox.innerHTML = `
    ...
    const resetFilters = () => {
        // Reset text search and dropdowns
        ...
        // Re-check all rarity checkboxes
        rarityCheckboxes.querySelectorAll("input[type='checkbox']").forEach(checkbox => {
            checkbox.checked = true;
        });

        // Rebuild the character dropdown to show all options again
        rebuildDropdown(characterNameSelect, allCharacterNamesData);

        // Re-run the filter and render function
        applyFilters();
    };
