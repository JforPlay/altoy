document.addEventListener("DOMContentLoaded", () => {
  ...
  let allPollDataCache = {}; // Cache to store all poll data
  let isSorting = false;
  ...
  const reSortView = () => {
    ...
    const newPollBoxes = Array.from(pollContainer.children);
    newPollBoxes.forEach(box => {
  ...
  const fetchAndDisplayResults = (skinId) => {
    ...
    pollRef.get().then((doc) => {
      if (doc.exists) {
        const data = doc.data();
        allPollDataCache[skinId] = data; // Update cache
