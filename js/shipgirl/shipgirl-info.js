...
async function showMapsModal(shipName) {
    // Ensure full data is loaded
    if (!fullShipData) {
        showToast('데이터 로딩 중...', 'info');
        fullShipData = await fullShipDataPromise;
        if (!fullShipData) {
...
function setupMapsModalListeners() {
    const modal = document.getElementById('mapsModal');
    ...
    if (searchInput) {
        searchInput.addEventListener('input', (e) => {
            ...
            const ship = fullShipData ? fullShipData.find(s => s.name === shipName) : shipgirlData.find(s => s.name === shipName);
...
function populateMapBrowserFilters() {
    ...
    const uniqueShipTypes = [...new Set(fullShipData.map(ship => String(ship.type)))].sort((a, b) => parseInt(a) - parseInt(b));
    ...
    const uniqueNationalities = [...new Set(fullShipData.map(ship => String(ship.nationality)))].sort((a, b) => parseInt(a) - parseInt(b));
...
function setupMapBrowserListeners() {
